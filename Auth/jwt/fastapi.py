...
class JWTAuth:
    ...
    async def current_user(self,request:Request,creds:Optional[HTTPAuthorizationCredentials] = Depends(HTTPBearer(auto_error=False))) -> Dict[str,Any]:
        try:
            token = None
            ...
            return await verify_access_token(token,self.settings,jwks_client=self.jwks)
